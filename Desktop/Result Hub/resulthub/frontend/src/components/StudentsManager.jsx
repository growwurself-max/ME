 ...
 */
export default function StudentsManager({ mode }) {
  const toast = useToast();
  const isResults = mode === 'results';

  const [courses, setCourses] = useState(null);
  const [exams, setExams] = useState(null);
  const [filters, setFilters] = useState({ course_id: '', section_id: '', exam_id: '', status: '', search: '', sort_by: 'rank', sort_order: 'asc' });
  ...
  const openAdd = () => {
    ...
    setForm({
      ...
      roll_number: '', hall_ticket_number: '', name: '',
  ...
  return (
    ...
    >
      <div className="card mb-4 grid gap-3 p-4 lg:grid-cols-6">
        ...
        <Field label="Search">
          <input className="input" placeholder="Roll number, hall ticket or name"
      ...
      {isResults && (
        <div className="card mb-4 flex flex-wrap gap-2 p-4">
          ...
          <button className="btn-primary" onClick={() => setPublished(true)}>Publish results</button>
            ...
            <table className="w-full">
              <thead className="bg-slate-50 dark:bg-slate-800/60">
                <tr>
                  ...
                  {course?.enable_pass_fail && <th className="th">Result</th>}
              ...
              <tbody className="row-divider">
                {students.map((student) => (
                  <tr key={student.id}>
                    <td className="td font-mono text-xs">{student.roll_number}</td>
                    ...
                    {course?.enable_pass_fail && <td className="td"><StatusBadge status={student.status} /></td>}
                    ...
                    <td className="td">
                      <div className="flex justify-end gap-1">
                        ...
                        <button className="btn-ghost text-rose-600" onClick={() => setConfirm(student)}>Delete</button>
                      </div>
                    </td>
                  </tr>
                ))}
          ...
          <form onSubmit={save} className="space-y-4">
            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="Roll number">
                <input className="input" required value={form.roll_number}
