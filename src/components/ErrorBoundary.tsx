...
export class ErrorBoundary extends React.Component<Props, State> {
  ...
  render() {
    if (this.state.hasError) {
      ...
      return (
        <div className="flex min-h-[420px] w-full flex-col items-center justify-center gap-4 rounded-2xl border border-white/10 bg-[#0f1015] p-8 text-center">
          ...
          <h3 className="font-display text-lg text-[#F4EDE2]">3D viewer unavailable</h3>
          ...
          <div className="flex gap-3 pt-2">
            <button
              ...
              className="rounded-full bg-[#B88E52] px-5 py-2 text-sm font-medium text-white hover:bg-[#A3704C] transition-colors"
            >
              Try again
            </button>
            <a
              href="#gallery"
              className="rounded-full border border-white/15 px-5 py-2 text-sm text-white/80 hover:bg-white/10 transition-colors"
            >
              Browse gallery
            </a>
          </div>
          {(import.meta as any).env?.DEV && this.state.error && (
            <pre className="mt-4 max-h-32 w-full overflow-auto rounded-lg bg-white/5 p-3 text-left text-[11px] leading-relaxed text-white/40">
