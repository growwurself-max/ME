...
export function ModelSkeleton({ label = 'Loading 3D…' }: { label?: string }) {
  return (
    <Html center>
      <div className="flex flex-col items-center gap-3 rounded-2xl border border-white/10 bg-[#0f1015]/70 px-6 py-5 backdrop-blur-md">
        <div className="h-8 w-8 animate-spin rounded-full border-2 border-[#B88E52]/30 border-t-[#B88E52]" />
        <p className="text-xs tracking-[0.18em] text-white/60 uppercase">{label}</p>
        <div className="h-1 w-24 overflow-hidden rounded-full bg-white/10">
          <div className="h-full w-1/2 animate-[shimmer_1.2s_infinite] bg-gradient-to-r from-transparent via-white/20 to-transparent" />
        </div>
      </div>
    </Html>
  )
    ...
    <Html center>
      <div className="flex flex-col items-center gap-4">
        <div className="h-12 w-12 rounded-full border border-white/10 flex items-center justify-center">
          <div className="h-6 w-6 animate-spin rounded-full border-2 border-white/20 border-t-[#B88E52]" />
        </div>
        <p className="text-[11px] tracking-[0.24em] text-white/40 uppercase">Preparing showroom</p>
      </div>
    </Html>
  )
}
